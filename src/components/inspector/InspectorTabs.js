 ...
 *
 * Alternative tabbed navigation with custom icons.
 *
...
const TABS = [
	{
		name: 'general',
		label: __( 'General', 'gambol-builder' ),
		icon: GeneralIcon,
	},
	{
		name: 'style',
 ...
 * @return {JSX.Element} Tabs element.
 */
const InspectorTabs = ( { activeTab = 'general', onTabChange, children } ) => {
	...
	return (
		<div className="gambol-inspector-tabs-wrapper">
			<div className="gambol-inspector-tabs-nav" role="tablist">
				{ TABS.map( ( tab ) => {
					...
					return (
						<button
							...
							aria-controls={ `gambol-tab-panel-${ tab.name }` }
							className={ `gambol-inspector-tab-btn ${ isActive ? 'is-active' : '' }` }
			...
			<div className="gambol-inspector-tabs-content">
				{ TABS.map( ( tab ) => (
					<div
						key={ tab.name }
						id={ `gambol-tab-panel-${ tab.name }` }
						role="tabpanel"
						aria-labelledby={ tab.name }
						className={ `gambol-inspector-tab-panel ${ currentTab === tab.name ? 'is-visible' : '' }` }
						hidden={ currentTab !== tab.name }
					>
						{ children && children[ tab.name ] }
					</div>
				) ) }
			</div>
		</div>
